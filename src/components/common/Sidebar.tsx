...
const links = [
  ...
  },
  {
    ...
    label: "Health",
    icon: HeartPulse,
    roles: ["health_actor", "admin"],
  },
  {
    href: "/dashboard/scan-dechets",
    label: "Scan déchets",
    icon: Camera,
    roles: ["farmer", "industry", "health_actor", "admin"],
  },
...
export function Sidebar({ role = "admin" }: { role?: UserRole }) {
  return (
    <aside className="hidden min-h-screen w-64 border-r border-slate-200 bg-white p-5 lg:block">
      ...
      >
        <span className="rounded-md bg-emerald-600 p-2 text-white">
          <Activity size={18} />
        </span>
        AgriConnect Smart
      </a>
      ...
      <div className="mt-8 rounded-lg bg-sky-50 p-4 text-sm text-sky-900">
        ...
        <p className="mt-1 text-sky-800">
          RLS, anomaly logs, and blockchain-style hashes are wired for demo
          flow.
        </p>
      </div>
    </aside>
